...
public class ScoreboardUtil {
    ...
    public static void updateScoreboard(ITeam clan) {
        Bukkit.getScheduler().runTaskAsynchronously(QTeam.getPlugin(), () -> {
            Scoreboard scoreboard = Bukkit.getScoreboardManager().getMainScoreboard();
            Team team = scoreboard.getTeam(clan.getTeamTag());
            if (team == null) {
                team = scoreboard.registerNewTeam(clan.getTeamTag());
            }
    ...
    public static void updateScoreboard(){
        if(QTeam.getConfigManager().getConfig().setClanTagTabPrefix()){
            Scoreboard scoreboard = QTeam.getClansScoreboard();
